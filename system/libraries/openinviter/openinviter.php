...
class Openinviter
	...
	public function startPlugin($plugin_name,$getPlugins=false)
		...
		elseif ($this->settings['hosted'])
			{
			if (!file_exists($this->basePath."/plugins/_hosted.plg.php")) $this->internalError=_X4OI_INVALID_PROVIDER;
			else
				{
		...
		elseif (file_exists($this->basePath."/plugins/{$plugin_name}.plg.php"))
			...
			if (file_exists($this->basePath."/conf/{$plugin_name}.conf")) 
				...
				if (empty($enable)) $this->internalError=_X4OI_INVALID_PROVIDER;
		...
		else { $this->internalError=_X4OI_INVALID_PROVIDER;return false; }
		return true;
		}
	...
	private function checkLoginCredentials($user)
		...
		if ($this->currentPlugin['requirement'])
			...
			if ($this->currentPlugin['requirement']=='email' AND !$is_email)
				{
				$this->internalError=X4OI_FULL_EMAIL_ADDRESS;
				return false;
				}
			elseif ($this->currentPlugin['requirement']=='user' AND $is_email)
				{
				$this->internalError=_X4OI_ONLY_USERNAME;
				return false;
				}
			}
		if ($this->currentPlugin['allowed_domains'] AND $is_email)
			...
			if (!$temp)
				{
				$this->internalError='<b>'.$user_domain.'</b> '._X4OI_BAD_DOMAIN;
				return false;
				}
			}
		return true;
