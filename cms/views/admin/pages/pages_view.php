			...
			if (($i->level > 1 && $i->xlock == 0) || $i->level == 4) {
				...
				if ($i->level > 2)
				{
					...
					if ($i->level == 4)
					{
						...
						if (ADVANCED_EDITING)
						{
							$actions .= ' <a class="btm" href="'.BASE_URL.'sections/index/'.$i->id_area.'/'.$i->id.'" title="'._SECTIONS.'"><i class="fas fa-bars fa-lg"></i></a>';
						}
...
window.addEvent('domready', function() {
	X3.content('filters', 'pages/filter/<?php echo $id_area.'/'.$lang.'/'.$xfrom ?>', '<?php echo X4Theme_helper::navbar($navbar, ' . ', false) ?>');
...
foreach ($sort as $k => $v)
{
	echo 'var sect'.$k.' = sortableLists.serialize('.($k).').filter(function(val) { return val !== null; });
        if ($("sort'.$k.'") != null) {
			$("sort'.$k.'").set("value", sect'.$k.');
		}
        tmp.push("'.substr($v, 2).'-" + sect'.$k.');';
}
	echo 'var req = new Request.HTML({
			url: root+"menus/menu/"+id+"/"+holder+"/"+tmp.join("_"),
					method:"get"
			}).send();
		}
	});';
...
<script>
window.addEvent("domready", function() {
	X3.content("filters", "pages/filter/'.$id_area.'/'.$lang.'/'.$page->url.'", "'.addslashes(X4Theme_helper::navbar($navbar, ' . ')).'");
	linking("ul.inline-list a");
	buttonize("topic", "btm", "topic");
