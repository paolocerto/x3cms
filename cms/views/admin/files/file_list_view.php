...
if (!empty($items[0])) 
{
	...
	foreach($items[0] as $i)
	{
		...
		{
				$actions = '<a class="bta" href="'.BASE_URL.'files/edit/'.$i->id.'" title="'._EDIT.'"><i class="fa fa-pencil fa-lg"></i></a>';
				...
				if ($i->level == 4)
				{
					$delete = '<a class="btl" href="'.BASE_URL.'files/set/xlock/'.$i->id.'/'.(($i->xlock+1)%2).'" title="'._STATUS.' '.$lock.'"><i class="fa fa-'.$lock_status.' fa-lg"></i></a> 
					<a class="bta" href="'.BASE_URL.'files/delete/'.$i->id.'" title="'._DELETE.'"><i class="fa fa-trash fa-lg red"></i></a>';
				}
		}
