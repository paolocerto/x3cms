...
class Theme_model extends X4Model_core
{
	 ...
	 */
	public function get_installed($xon = 2)
	{
		// condition
	 ...
	 * @return  array	Array of errors
	 */
	public function uninstall($id_theme, $name)
	{
		$error = array();
	 ...
	 * @return  integer
	 */
	private function exists($name, $id = 0)
	{
		// condition
		$where = ($id) 
			? ' AND id <> '.intval($id)
	 ...
	 * @return  array	Array of objects
	 */
	public function get_css($id_theme)
	{
		return $this->db->query('SELECT DISTINCT css 
			FROM templates
			WHERE id_theme = '.intval($id_theme));
	}
	 ...
	 * @return  array	Array of objects
	 */
	public function get_js($id_theme)
	{
	 ...
	 * @return  string
	 */
	public function compress_css($str)
	{
		// Remove comments
