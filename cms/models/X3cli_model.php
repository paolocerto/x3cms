...
function redirect() {
	 ...
	 * @return  void
	 */
	public function index($id_area = 2, $lang = \'en\', $pp = 0, $str = \'\')
	{
	    // load the dictionary
		// you should have created a section in the dictionary for this controller
		...
		{
		    $view->content->items = $mod->get_items($id_area, $lang, $str);
		}
		else
		{
		    // if not sortable we paginate
		    $view->content->items = X4Pagination_helper::paginate($mod->get_items($id_area, $lang, $str), $pp);
		}
	 ...
	 * @return  void
	 */
	public function filter($id_area, $lang, $str = \'\')
	{
		// load the dictionary
		$this->dict->get_wordarray(array(\''.$name.'\'));
...
window.addEvent("domready", function()
{
	 ...
	 * @return  void
	 */
	private function deleting($obj, $_post)
	{
		$msg = null;
		// check permission
	 ...
	 * @return  void
	 */
	public function ordering($id_area, $lang) 
	{
		$msg = null;
		// check permission
	 ...
	 * @return	array	string
	 */
	public function rewidget($title, $id_area, $area)
	{
		$mod = new '.$uname.'_model();
...
class '.$uname.'_controller extends X4Cms_controller
{
	 ...
	 * @return  boolean
	 */
	public function create_model($area, $name)
	{
	    $uname = ucfirst($name);
...
class '.$uname.'_model extends X4Model_core
{
	...
	*/
	 ...
	 */
	public function get_items($id_area, $lang, $str = \'\')
	{
		$where = \'\';
		if (!empty($str))
		{
	 ...
	 * @return  boolean
	 */
	public function is_sortable($id_area, $lang)
	{
	 ...
	 * @return  array	array of objects
	 */
	public function get_max_pos($id_area, $lang) 
	{
	 ...
	 * INSERT INTO modules (updated, id_area, name, description, configurable, admin, searchable, mappable, widget, version, xon) VALUES (NOW(), ID_AREA, \''.$name.'\', \''.$uname.' widget\', 0, 0, 0, 0, 1, \'0\', 1);
...
class '.ucfirst($name).'_obj 
{
	 ...
	 * @return  boolean
	 */
	public function create_view($area, $name)
	{
...
<p>TEXT</p>';
	 ...
	 * @return  booelan
	 */
	private function check_privtypes($name)
	{
	    // get admin languages
	 ...
	 * @return  boolean
	 */
	public function create_dictionary($area, $name)
	{
	    $uname = strtoupper($name);
