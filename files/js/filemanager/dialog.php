...
else
{
	...
	if ($subdir == '' && !empty($_COOKIE['last_position']) && strpos($_COOKIE['last_position'],'.') === FALSE)
	{
	...
	***/
	...
	$popup = (isset($_GET['popup']))
		? strip_tags($_GET['popup'])
		: 0;

	//Sanitize popup
	$popup = !!$popup;

	$crossdomain = (isset($_GET['crossdomain']))
		? strip_tags($_GET['crossdomain'])
		: 0;
    ...
    $field_id = isset($_GET['field_id'])
        ? fix_get_params($_GET['field_id'])
        : '';
...
<?php
	...
	if ($aviary_active)
    {
        ...
        {
	        echo '<script type="text/javascript" src="https://dme0ih8comzn4.cloudfront.net/js/feather.js"></script>';
        }
        else
        {
...
?>
				<form action="/files/js/filemanager/dialog.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone" class="dropzone">
...
<?php
        if ($upload_files)
        {
            echo '<button class="tip btn upload-btn" title="'.lang_Upload_file.'"><i class="rficon-upload"></i></button>';
        }

        if ($create_text_files)
        {
            echo '<button class="tip btn create-file-btn" title="'.lang_New_File.'"><i class="icon-plus"></i><i class="icon-file"></i></button>';
        }

        if ($create_folders)
        {
            echo '<button class="tip btn new-folder" title="'.lang_New_Folder.'"><i class="icon-plus"></i><i class="icon-folder-open"></i></button>';
        }

        if ($copy_cut_files || $copy_cut_dirs)
        {
            echo '<button class="tip btn paste-here-btn" title="'.lang_Paste_Here.'"><i class="rficon-clipboard-apply"></i></button>
...
<!-- header div end -->
    ...
    <div class="row-fluid">
	<?php
	$link="/files/js/filemanager/dialog.php?".$get_params;
	?>
	    <ul class="breadcrumb">
...
<?php
    ...
    if (!empty($bc))
    {
	    foreach ($bc as $k => $b)
        {
            ...
            elseif ($b != '')
            {
                echo '<li><a href="'.$link.$tmp_path.'">'.$b.'</a></li>
                    <li><span class="divider">/</span></li>';
            }
	    }
    }
...
<?php
        ...
        {
            echo '<div class="alert alert-block">'.$folder_message.'</div>';
	    }
...
<?php
        ...
        foreach ($files as $file_array)
        {
            ...
            // build the item for folders
            $li_class = ($file == '..')
                ? 'class="back"'
                : 'class="dir"';

?>
                <li data-name="<?php echo $file ?>" <?php echo $li_class ?>>
...
<?php
                if ($show_folder_size)
                {
                    echo '<div class="file-size">'.makeSize($file_array['size']).'</div>';
                }
?>
                        ...
                        <figcaption>
                            <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($rename_folders && !$file_prevent_rename) echo "rename-folder"; ?>" title="<?php echo lang_Rename?>" data-path="<?php echo $rfm_subfolder.$subdir.$file; ?>" data-thumb="<?php echo $thumbs_path.$subdir.$file; ?>">
                                <i class="icon-pencil <?php if(!$rename_folders || $file_prevent_rename) echo 'icon-white'; ?>"></i>
                            </a>
...
<?php
        ...
        foreach ($files as $nu => $file_array)
        {
            ...
            // is an image?
            if (file_exists($file_path) && in_array($extension_lower, $ext_img))
            {
                ...
                if (!file_exists($src_thumb))
                {
                    try
                    {
                        ...
                        new_thumbnails_creation(
                            $current_path.$rfm_subfolder.$subdir,
                            $file_path,
                            $file,$current_path,
            ...
            if (
                (
                    !($_GET['type' ] == 1 && !$is_img) &&
                    !(
                        ($_GET['type'] == 3 && !$is_video) &&
                        ($_GET['type'] == 3 && !$is_audio)
                    )
                ) && $class_ext > 0
            )
            {
...
?>
                            ...
                            <div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
                                <div class="filetype <?php echo $extension_lower ?> <?php if(in_array($extension_lower, $editable_text_file_exts)) echo 'edit-text-file-allowed' ?> <?php if(!$is_icon_thumb){ echo "hide"; }?>">
