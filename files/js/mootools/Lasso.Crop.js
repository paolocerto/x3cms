...
Lasso.Crop = new Class({
	...
	options : {
		...
		handleSize : 8,
		preset : false,
		handleStyle : { 'border' : '1px solid #eee', 'background-color' : '#eee' , opacity : .75  },
		zoom: 1
	},
	...
	initialize : function(img,options){
		...
		var image_editor_width = windowWidth();	
		var coords = this.img.getCoordinates();
		
		if (coords.width > image_editor_width - 350) {
			coords.width = Math.round(coords.width/2);
			coords.height = Math.round(coords.height/2);
			options.zoom = 2;
			
			// update preset
			this.options.preset[2] = coords.width;
			this.options.preset[3] = coords.height;
		}

		this.container = new Element('div',{
			'styles' : { 'position' : 'relative', 'width' : coords.width, 'height' : coords.height, 'background' : 'url('+this.img.get('src')+') no-repeat', 'background-size': 'cover' }
	...
	reclip : function(x0, y0, x1, y1, k){
		...
		if (k == 0 && x0 > 0) d[0] = Math.round(x0/this.options.zoom);
		else d[0] = box.x[0];
		if (k == 1 && y0 > 0) d[1] = Math.round(y0/this.options.zoom);
		else d[1] = box.y[0];
		
		if (k == 2 && x1 < this.options.preset[2]*this.options.zoom) d[2] = Math.round(x1/this.options.zoom) + d[0]; 
		else d[2] = box.x[1];
		if (k == 3 && y1 < this.options.preset[3]*this.options.zoom) d[3] = Math.round(y1/this.options.zoom) + d[1];
		else d[3] = box.y[1];
