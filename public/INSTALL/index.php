...
// DEFINE THE CMS PATH
define('CMS_ROOT', str_replace('/public/INSTALL/', '/cms/', PATH));
 ...
 */
function get_queries(string $file) : array
{
	$a = [];
 ...
 */
function get_timezone(string $value) : string
{
	$o = '';
...
<head lang="en">
    ...
    <link type="text/css" rel="stylesheet" href="../themes/admin/css/fa.all.min.css">
    <link type="text/css" rel="stylesheet" href="../themes/admin/css/x3ui.css">

    <link type="text/css" rel="stylesheet" href="./tailwind_installer.css">
...
switch($step)
{
	...
	case 2:
        ...
        switch ($web_server)
        {
            ...
            case 'Nginx':
                ...
                break;
            default:
                $web_server = FAILED.' You need Apache or Nginx web server to run X3 CMS';
                $check = false;
                break;
	...
	case 3:
		...
		foreach ($perms as $i => $perm)
		{
			if (!empty($perm))
			{
                ...
                if ($web_server != 'Apache' && $perm == '.htaccess')
                {
                    $perm = 'index.php';
                }
	...
	case 4:
        ...
        if (isset($_POST) && !empty($_POST))
        {
            ...
            if (empty($msg))
			{
				...
				try
				{
					$db = new PDO($dsn, $_POST['dbuser'], $_POST['dbpass'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
			...
			if (empty($msg))
			{
				...
				if ($check_config)
				{
					...
					$tmp = md5(time());
					$_SESSION['tmp'] = $tmp;
					$check_sql = file_put_contents(CMS_ROOT.'files/'.$tmp, $sql);
	...
	case 5:
		...
		$db = new PDO($dsn, $_SESSION['dbuser'], $_SESSION['dbpass'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
		$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		// get sql
		$query = get_queries(CMS_ROOT.'files/'.$_SESSION['tmp']);
		...
		{
			// delete db tmp file
			unlink(CMS_ROOT.'files/'.$_SESSION['tmp']);
		...
		if (isset($_REQUEST['r']) && $_REQUEST['r'] == 1)
		{
            ...
            switch ($web_server)
            {
                case 'Apache':
                    ...
                    $domain_prefix = str_replace(['http://', 'https://', '.', ':'], '', $domain);
                    $file = str_replace(array('HTADOMAIN', 'HTAROOT', 'SITEMAP_PREFIX', 'WWWDOMAIN'), array($domain, $hta_root, $domain_prefix, $www), $txt);
                    $check = @file_put_contents (FINAL_ROOT.'.htaccess', $file);
                    $error = '.htaccess';
                    break;
                case 'Nginx':
                    $page = @file_get_contents(FINAL_ROOT.'page.php');
                    $check = @file_put_contents (FINAL_ROOT.'index.php', $page);
                    $error = 'index.php';
                    break;
            }
			...
			if ($check && $check2)
			{
				...
				@chmod(CMS_ROOT.'config/config.php', 0755);
			...
			else
			{
				$body .= '<h3>Installation aborted</h3>
					<p class="failed px-4 py-4 rounded">Something went wrong while updating '.$error.' file.<br />Check it and try again.</p>';
			}
		}
		else
		{
