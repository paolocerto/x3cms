...
// breadcrumb
$breadcrumb = array($this->site->get_bredcrumb($page));
...
else
{
    ...
    echo '<div class="small_container mx-auto">
            <div class="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6 px-4 pb-2">
                <div class="lg:col-span-2">';
    // section1
    $v = array_shift($sections);
    // sectionize(&$css, $site, $page, $args, $index, $section, $grid = '')
    echo X4Theme_helper::sectionize($css, $this->site, $page, $args, 1, $v, 'tailwind');
    echo '</div>';
    // section2
    if (!empty($sections))
    {
        ...
        $v = array_shift($sections);
        // sectionize(&$css, $site, $page, $args, $index, $section, $grid = '')
        echo X4Theme_helper::sectionize($css, $this->site, $page, $args, 2, $v, 'tailwind');
        echo '</div>';
    }
    echo '</div></div>';
...
?>
    ...
    <footer class="py-8">
        <p class="text-center text-sm">
            ...
            <a href="http://www.x3cms.net/en/x3-cms-legal-notices" title="X3 CMS Legal Notices">X3 CMS Legal Notices</a>
        </p>
<?php
if (DEBUG)
{
